...
class SQLService {
  ...
  Future saveRecord(ShopItemModel data) async {
    await db.transaction((txn) async {
      var qry =
          'INSERT INTO shopping(name, price, image,rating,fav,description) VALUES("${data.name}",${data.price}, "${data.image}",${data.rating},${data.fav != null ? 1 : 0},"${data.description}")';
  ...
  Future addToCart(ShopItemModel data) async {
    await db.transaction((txn) async {
      var qry =
          'INSERT INTO cart_list(shop_id, name, price, image,rating,fav,description) VALUES(${data.id}, "${data.name}",${data.price}, "${data.image}",${data.rating},${data.fav != null ? 1 : 0},"${data.description}")';
      int id1 = await txn.rawInsert(qry);
      return id1;
    });
  }
